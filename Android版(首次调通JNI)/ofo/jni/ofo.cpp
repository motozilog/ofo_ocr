...
using namespace cv;
using namespace std;
static float feature2Extract[7][32/8*72/8];
static int resultOfoOcr[7];
...
//JNImain
extern "C" {
JNIEXPORT jintArray JNICALL Java_com_scut_ofo_ofo_grayProc  (JNIEnv* env, jclass obj, jintArray buf, jint w, jint h,jint ror) //请改成自己
{
    jint *cbuf;
...
//        int q = svm->predict(trainningDataMat);
    ...
    jintArray jarr = env->NewIntArray(7);
