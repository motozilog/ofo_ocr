...
//                (Mat(contoursQRFind[i]).size())>800)
    ...
    //start 传入YUV时需要为偶数
    int cropQRx = distanceQRx;
    int cropQRy = distanceQRy;
    if(((cropQRx)%2)==1)
    {
    	cropQRx ++;
    }
    ...
    cv::Rect maskOfoQR(QRx, QRy, cropQRx, cropQRy);
    cv::Mat croppedOfoQR(rotateOfoImg, maskOfoQR);
    cv::imwrite("/sdcard/ofo_ocr/qr.jpg",croppedOfoQR);
